...
function drawRightPill(ctx, rightX, cy, iconKey, text, accent) {
    ctx.font = '20px FredokaOne';
    const iconSize = 24;
    const gap = 8;
    const padX = 14;
...
function drawHeader(ctx, player, prestige) {
    ctx.font = '46px FredokaOne';
    const grad = ctx.createLinearGradient(PAD, 30, PAD + 460, 30);
    ...
    ctx.lineWidth = 5;
    ctx.strokeText('Stand Upgrades', PAD, 74);
    ctx.fillStyle = grad;
    ctx.fillText('Stand Upgrades', PAD, 74);

    ctx.font = '19px FredokaOne';
    ctx.fillStyle = COLOURS.subtitle;
    ctx.fillText('Level up your stand', PAD + 2, 104);

    drawRightPill(ctx, WIDTH - PAD, 50, 'prestige', `Prestige ${prestige}`, COLOURS.title);
    drawRightPill(ctx, WIDTH - PAD, 96, 'cash', `$${formatNumber(player?.economy?.cash ?? 0)}`, '#2E8B39');
...
function drawUpgradeCard(ctx, x, y, w, h, stat, player, prestige) {
    ...
    drawIconBadge(ctx, cx, y + 50, 32, UPGRADE_ICON_KEYS[stat]);
    ...
    drawCostPill(ctx, cx, y + 204, stat, level, prestige);
}

function drawFooter(ctx, cy) {
    ctx.font = '18px FredokaOne';
    const cmd = '/upgrade buy';
    const rest = ' to buy more upgrades';
    const total = ctx.measureText(cmd).width + ctx.measureText(rest).width;
...
export async function renderUpgrades(player) {
    await preloadIcons([...Object.values(UPGRADE_ICON_KEYS), 'cash', 'prestige']);
